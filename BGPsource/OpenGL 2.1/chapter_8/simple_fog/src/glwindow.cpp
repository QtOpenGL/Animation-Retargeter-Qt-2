...
LRESULT GLWindow::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        ...
        if (wParam == VK_SPACE) //If the space key was pressed
        {
		std::string newTitle = getAttachedExample()->toggleFogMode();
		setWindowCaption(newTitle);
        }
    break;
    default:
        break;
