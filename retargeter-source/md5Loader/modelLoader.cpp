...
AnimEntity * modelLoader::loadModel(string filePath){
    ...
    if(infile.fail())
        throw MD5Exception("Unable to load Mesh: ", filePath);
...
Skeleton modelLoader::populateJoints(ifstream &infile){
    ...
    /*for(int i = 0; i < bindPose.size(); i++){
        cout << i << ": " << bindPose[i].getName() << " height: " << bindPose[i].getHeight() << endl;
    }*/
    
    for(int i = 0; i < bindPose.getHeights().size(); i++){
        cout << bindPose.getHeights()[i] << endl;
    }
